...
**Note**: The docker container may already have the required packages pre-installed. These instructions have been provided in case someone needs to know how to fix their setup in the future if they mess it up (probably one of the devs :P).

If you're following this tutorial to execute autonomous navigation on a real car, you'd need to make the on-board computer (either a jetson nano or a jetson Xavier NX) is connect to the internet. You may have to connect your system to a monitor to do this as the default setting is for the on-board computer to act as a hotspot. Once you've connected your system to the internet, you'd need to find the IP address of the car,
...
Alternately, you could use a keyboard, mouse and monitor to do the same job without tearing your hair out.

Downloading the package for RHC, localization and planning:
{{< highlight bash >}}
...
# installing sbpl for the global planner
...
The reason why there are two planners (mushr_gp and mushr_gprm) is that mushr_gp happens to be too resource intensive to be run on the jetson nano 4GB variant. For the jetson nano, we recommend using the mushr_gprm package. For the Jetson Xavier NX or running on a laptop/desktop, mushr_gp should work correctly.
...
**Note**: You also need to download all dependencies packages for the [`mush_rhc`](https://github.com/prl-mushr/mushr_rhc).
...
**Note:** If you're operating on the robot, you'd need to SSH into it. If you're operating in sim, you can just open a new terminal and skip the tmux related commands.
...
{{</highlight>}}
where you'd replace `CONTAINER_ID` with your container ID.
