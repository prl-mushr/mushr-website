...
First off, nvidia-docker/docker is pre-installed for the specific hardware using the NVIDIA image we provided earlier, but the Docker Compose v2 plugin may not be. The MuSHR install script (run in the next step with the robot option) installs `docker-compose-plugin` for you and registers the NVIDIA container runtime. If you need to install Compose manually:
...
Fortunately, the steps for installing the MuSHR Docker Container on the car are similar to installing the container on a Desktop device. Please follow the section titled **Installing MuSHR Docker Container** from the [humble quickstart tutorial](/tutorials/humble_quickstart).
...
Unlike simulation, visualization usually is run on a separate computer (though you can install Foxglove on the robot). From the [humble quickstart tutorial](/tutorials/humble_quickstart) complete the following steps on your separate visualization computer.
...
First, all ROS commands must be run inside a docker container. Docker containers are created via the `mushr_humble` command. If you need to create multiple terminal sessions we recommend entering one docker container then using tmux (pre-installed). That way you only need to remember to run `mushr_humble` once.

Second, the code itself resides in `~/colcon_ws` and is attached to the docker container. This allows you to use your favorite editor and tweak code outside the container. Code edited outside the container will automatically be updated inside the container so there is no need to restart a container.
...
Then run teleop
{{< highlight bash >}}
$ ros2 launch mushr_base teleop.launch.py foxglove_teleop:=1
{{< / highlight >}}
...
## Tuning
If your car does not drive in a straight line or you run into other similar issues, please check out the following [tuning](/tutorials/tuning/) tutorial.
